...
def save():
    if not config.get('images') and not config.get('links'):
...
def save_json(filename):
    if filename:
        ...
        for img in config.get('images', []):
            ...
            data[img['name']] = b64_img_data
        data['links'] = config.get('links', [])
...
if __name__ == "__main__":
    _root = Tk()
    _root.title('Enhanced SEO Scraper App')
    ...
    _images = StringVar()
    _img_listbox = Listbox(_img_frame, listvariable=_images, height=6, width=50)
    ...
    _choice_lbl = ttk.Label(_radio_frame, text="Choose how to save content")
